...
# This part is just exploratory not necessary 
...
x = data.frame(tsne.rot, as.factor(clustering)); colnames(x)<- c("tSNE_1","tSNE_2","clustering")
ggplot(x, aes(x=tSNE_1, y=tSNE_2, color=clustering)) + geom_point() + scale_color_manual(values=brewer16)




## Visualize expression of single gene (as feature plot of Seurat)
#Plot all notch genes as feature plot:
notch.all<-c( "Dll1",   "Dll3"   ,"Dll4"   ,"Dtx1",   "Jag1"   ,"Jag2",   "Notch1","Notch2" ,"Notch3", "Notch4" ,"Mfng",   "Rfng",   "Lfng")

featureListPlot<-function(tpm_matrix = atlas_tpm_norm, gene.list,main.title= "Gene expression", fig.cols = 5){
    p = list()
    for(i in 1:length(gene.list)){
        which.gene= gene.list[i]
        x$gene =as.vector(t(tpm_matrix[which.gene,]))
        p[[i]] =  ggplot(x, aes(x=tSNE_1, y=tSNE_2)) + geom_point(aes(colour= gene))  + scale_color_gradient(low = "gray69",high = "maroon2")  + ggtitle(gene.list[i])
    }

    x11()

    grid.arrange(grobs=p,ncol = fig.cols, top = main.title)

}



## Analyse Full-length data set (1.5k cells)
# Data is already normalized by TPM (not log!)
atlas_full = read.delim("GSE92332_AtlasFullLength_TPM.txt.gz")
#take bmp genes
bmp_recep_fullAtlas = atlas_full[bmp.receptors,]
bmp_ligands_fullAtlas = atlas_full[bmp.ligands,]
